...
from torch.nn import Module, Sequential, ReLU, Hardswish, Conv2d, BatchNorm2d, Dropout, Linear, Hardsigmoid

class SqueezeExcite(Module):
    def __init__(self, expand_channels, reduction=4) -> None:
        super().__init__()
...
class Bottleneck(Module):
    def __init__(self, in_channels, out_channels, expand_channels, kernel_size, activation, use_se, stride) -> None:
        super().__init__()
        self.use_se = use_se
        ...
        self.dconv1 = Sequential(
            Conv2d(expand_channels, expand_channels, kernel_size=kernel_size, stride=stride, padding=padding, groups=expand_channels, bias=False),
            BatchNorm2d(expand_channels),
        )
...
class CustomMobileNetSmall(Module):
    def __init__(self, n_classes, dropout=0.2):
        ...
        for inp, out, exp, k, act, se, s in layers:
            self.invRes.append(Bottleneck(inp, out, exp, k, act, se, s))
